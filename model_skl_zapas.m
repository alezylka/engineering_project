function dydt = model_skl_zapas(t, y, ATPi, ATPgli, maxGli, metabolizm, b1, b2, b3, c1, c2, c3, d1)
    ...
    if (t >= 60 && t <= 65) || (t >= 400 && t <= 420) || (t >= 660 && t <= 670)
        ...
        % linie zakomentowane do wartości diety dostarczające nadwyżkę
        % kalor
        if t >= 60 && t <= 65
            % dieta = 9.12;
            dieta = 12.12;
    ...
    if (t >= 60 && t <= 65) || (t >= 400 && t <= 420) || (t >= 660 && t <= 670)
        ...
        % energii)
        if Gli > maxGli
            dC16dt = abs(d1 * d1 * ((ATP * (E/I) * var * dIdt / 129) - d1 * d1 * C16 * (I / E)) * step_power_series);
        else
        ...
        if Gli > maxGli
            ...
            dC16dt = d1 * (ATP * (E/I) * var * dIdt / 129) - d1 * d1 * C16 * (I / E);
        end
